...
class Bus {
    constructor(TrackerId,InitialRoute,InitialPosition) {
        ...
        this.nextBusStop = null;      //stores the next bus stop when found
   ...
   // 
    updatePrevBusStop(busStopName) {
        ...
        if(busStopName.toUpperCase() === "UNKNOWN") {
            ...
            this.setButtonTextHTML("Unknown", "Unknown");
        }
        //else if on regular route
            ...
            if(this.prevBusStopName === 'CALCULATING') {
               this.setButtonTextHTML("Unknown", "TBD");
     ...
     * @returns {null}
     */
    setButtonTextHTML(nextBusStopName, eta) {
          document.getElementById(this.button.id).innerHTML = 
                    `Bus ${String(this.trackerid)}: ${this.route.name} 
                <div align="left"> ${nextBusStopName}  
            ...
            }, async function(response, status)
                {
                    if(status !== 'OK') {
                     ...
                     //   console.log("Distance, duraction: "+distance+", "+ duration);
                        _this.setButtonTextHTML(_this.nextBusStop.name, duration);
                    }
                });
    }     
}
